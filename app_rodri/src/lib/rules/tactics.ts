...
export function recommend(input: TacticsInput, snap: TacticsSnap): Plan {
  ...
  if (input.deadRecent.length) {
    ...
    const heal = preparedSlot(snap, 'heal');
    const sure = a?.hpCurrent !== null && a?.hpCurrent !== undefined ? a.hpCurrent + healAmount(11) > 0 : true;
    recs.push({ action: 'standard', title: `Heal + Relentless Healing sobre ${allyName(id)}`, detail: `La cura revive si supera sus pg negativos${a?.hpCurrent !== null && a?.hpCurrent !== undefined ? ` (está a ${a.hpCurrent}: ${sure ? 'queda con pg positivos, revive seguro' : 'quedaría en negativo: revive solo si supera su umbral (−Con, lo decide el DM); Mythic Heal 165 o Inspired 130 dan más margen'})` : ''}. Relentless es gratuita.`, numbers: `${healAmount(11)} pg`, cost: '1 ranura 6.º + 1 PM', available: !!heal && pm >= 1, reason: !heal ? 'Sin Heal preparado' : pm < 1 ? 'Sin poder mítico' : undefined, exec: heal ? { kind: 'cast', slotId: heal.id } : undefined, priority: 100 });
    ...
    recs.push({ action: 'standard', title: 'Breath of Life (ranura de dominio de 5.º)', detail: 'Alternativa si no hay Heal: revive si murió hace ≤ 1 asalto y la cura lo deja por encima de su umbral de muerte.', numbers: '5d8+11 +1 ≈ 34,5', cost: '1 ranura 5.º [D]', available: !!bol, reason: bol ? undefined : 'Sin Breath of Life preparado', exec: bol ? { kind: 'cast', slotId: bol.id } : undefined, priority: 85 });
